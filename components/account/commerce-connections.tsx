...
export default function CommerceConnections() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/ebay/settings?connection_id=${encodeURIComponent(ebayConnection.id)}`, { cache: 'no-store' })
      .then(async (res) => {
        const data = await res.json()
        if (!res.ok) throw new Error(data.error || 'Unable to load eBay settings')
        return data.settings
      })
      .then((settings) => {
        if (cancelled || !settings) return
        setEbay({
          marketplace_id: settings.marketplaceId || 'EBAY_US',
      ...
      .catch((err) => { if (!cancelled) setError(err.message || 'Unable to load eBay settings') })
