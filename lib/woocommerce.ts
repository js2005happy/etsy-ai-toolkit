...
function authHeader(credentials: WooCommerceCredentials) {
  if (!credentials.consumerKey?.startsWith('ck_') || !credentials.consumerSecret?.startsWith('cs_')) {
    throw new Error('Invalid WooCommerce REST API credentials')
  }
...
async function wooFetch(storeUrl: string, credentials: WooCommerceCredentials, path: string, init: RequestInit = {}) {
  ...
  try {
    return await fetch(`${base}/wp-json/wc/v3${path}`, {
      ...init,
      cache: 'no-store',
...
export async function createWooProduct(storeUrl: string, credentials: WooCommerceCredentials, input: WooProductInput): Promise<any> {
  ...
  if (Number.isInteger(input.stockQuantity) && Number(input.stockQuantity) >= 0) {
    payload.manage_stock = input.manageStock ?? true
    payload.stock_quantity = Number(input.stockQuantity)
  }
