  ...
  return (
    <div className="container mx-auto py-10 px-4 max-w-5xl">
      ...
      <div className="flex justify-between items-end mb-8">
        <div><h1 className="text-3xl font-bold font-display text-foreground">{t('dashboardTools.email.h1')}</h1><p className="text-muted-foreground">{t('dashboardTools.email.sub')}</p></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card>
          <CardHeader><CardTitle className="font-display">{t('dashboardTools.email.brief')}</CardTitle><CardDescription>{t('dashboardTools.email.briefDesc')}</CardDescription></CardHeader>
          <CardContent>
        ...
        <div className="space-y-6">
          {!result && !loading && !error && <div className="h-full flex flex-col items-center justify-center text-center p-12 border-2 border-dashed border-border rounded-xl text-muted-foreground"><p>{t('dashboardTools.email.empty')}</p></div>}
          {loading && <div className="h-full flex flex-col items-center justify-center p-12 text-center"><Loader2 className="h-12 w-12 animate-spin text-primary mb-4" /><p className="text-lg font-medium">{t('dashboardTools.email.loading')}</p></div>}
          {result && <Card><CardHeader className="space-y-0 pb-2"><div className="flex items-center justify-between"><CardTitle className="text-sm font-medium font-display">{t('dashboardTools.email.subject')}</CardTitle><Button variant="ghost" size="sm" onClick={() => handleCopy(result.subject, 'subject')}>{copiedField === 'subject' ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}</Button></div><p className="text-base font-medium">{result.subject}</p><p className="text-xs text-muted-foreground">{result.preview_text}</p></CardHeader><CardContent className="pt-2"><div className="flex items-center justify-end mb-2"><Button variant="ghost" size="sm" onClick={() => handleCopy(result.body, 'body')}>{copiedField === 'body' ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}</Button></div><div className="whitespace-pre-wrap text-sm leading-relaxed rounded-lg bg-secondary/50 p-4">{result.body}</div></CardContent></Card>}
        </div>
      </div>
    </div>
  )
