...
export default function DashboardPage() {
  ...
  const groups = [
    { label: t('home.groupCreate'), benefit: t('home.groupCreateBenefit'), tools: [
    ...
    { label: t('home.groupOptimize'), benefit: t('home.groupOptimizeBenefit'), tools: [
    ...
    { label: t('home.groupReply'), benefit: t('home.groupReplyBenefit'), tools: [
    ...
    { label: t('home.groupGrow'), benefit: t('home.groupGrowBenefit'), tools: [
  ...
  useEffect(() => {
    let cancelled = false
    fetch('/api/user/credits').then((res) => res.ok ? res.json() : null).then((data) => {
      if (cancelled || !data) return
      setCredits(data.credits); setPlan(data.plan ?? null); setQuota(data.quota ?? null); setImageCredits(data.imageCredits ?? null); setImageQuota(data.imageQuota ?? null)
  ...
  const handleUpgrade = async () => {
    setIsUpgrading(true)
    try { const res = await fetch('/api/paddle/checkout', { method: 'POST' }); if (!res.ok) throw new Error('checkout'); const data = await res.json(); window.location.href = data.url }
  ...
  const handleManageBilling = async () => {
    setIsManaging(true)
    try { const res = await fetch('/api/paddle/portal', { method: 'POST' }); if (!res.ok) throw new Error('portal'); const data = await res.json(); window.location.href = data.url }
    ...
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-6xl px-5 py-14 md:py-16">
        <header className="mb-10">
          ...
          <p className="mt-3 max-w-2xl text-lg text-muted-foreground">Find what needs attention, review AI changes, and publish only when you are ready.</p>
        </header>
        ...
        <section className="mb-8 flex flex-col gap-5 rounded-3xl border border-primary/20 bg-primary/5 p-6 md:flex-row md:items-center md:justify-between md:p-7">
          <div className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Sparkles className="h-5 w-5" /></div><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">Shop Voice</p><h2 className="mt-1 font-display text-2xl">Make every Craftly tool sound like the same shop.</h2><p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">Save your tone, audience, preferred vocabulary, words to avoid, product conventions and brand story once. Supported writing tools reuse it automatically.</p></div></div>
        ...
        <section className="relative mb-14 overflow-hidden rounded-3xl bg-primary p-8 text-primary-foreground md:p-10">
          ...
          <div className="relative flex flex-col items-start justify-between gap-8 md:flex-row md:items-center">
            <div><div className="flex items-center gap-2 text-sm font-medium text-primary-foreground/70"><Coins className="h-4 w-4" /> {t('dashboard.yourCredits')}</div><div className="mt-3 flex items-baseline gap-3"><span className="font-display text-5xl leading-none tracking-tight md:text-6xl">{credits !== null ? credits : '…'}</span>{quota !== null && <span className="text-lg text-primary-foreground/60">/ {quota}</span>}</div><div className="mt-4 h-2 w-64 overflow-hidden rounded-full bg-white/20"><div className="h-full rounded-full bg-white transition-all" style={{ width: `${progressPercentage}%` }} /></div><div className="mt-6 flex items-center gap-3 text-primary-foreground"><ImageIcon className="h-4 w-4" /><span className="font-medium">Images</span><span className="text-primary-foreground/70">{imageCredits ?? '…'} / {imageQuota ?? '…'}</span></div><div className="mt-2 h-2 w-64 overflow-hidden rounded-full bg-white/20"><div className="h-full rounded-full bg-white transition-all" style={{ width: `${imageProgressPercentage}%` }} /></div></div>
            {isPaid ? <Button variant="outline" className="rounded-full border-white/30 bg-white/10 px-6 py-3 font-medium text-primary-foreground hover:bg-white/20" onClick={handleManageBilling} disabled={isManaging}>{isManaging && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{t('dashboard.manageBilling')}</Button> : <Button className="rounded-full bg-white px-6 py-3 font-semibold text-primary hover:bg-white/90" onClick={handleUpgrade} disabled={isUpgrading}>{isUpgrading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{isUpgrading ? t('dashboard.processing') : t('dashboard.upgradeToPro')}</Button>}
          </div>
        </section>

        <div className="mb-8 flex flex-col justify-between gap-2 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Execution layer</p><h2 className="mt-1 font-display text-2xl tracking-tight text-foreground">Workspace tools</h2></div><span className="max-w-md text-sm text-muted-foreground sm:text-right">Use these when an action needs deeper writing, optimization, communication or growth work.</span></div>

        <div className="space-y-14">{groups.map((group) => <section key={group.label}><div className="mb-6 flex flex-wrap items-baseline justify-between gap-2"><h3 className="font-display text-xl tracking-tight text-foreground">{group.label}</h3><span className="text-sm text-muted-foreground">{group.benefit}</span></div><div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">{group.tools.map((tool) => <TiltCard key={tool.href} href={tool.href} icon={<tool.icon className="h-6 w-6" />} title={tool.title} description={tool.description} />)}</div></section>)}</div>
      </div>
    </div>
  )
}
